...
function directionPlan(style='automatico'){
  const groups={
    elegante:["zoompan=z='min(zoom+0.00055,1.065)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'","zoompan=z='if(eq(on,1),1.065,max(1.0,zoom-0.00052))':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'","zoompan=z='1.045':x='iw/2-(iw/zoom/2)':y='(ih-ih/zoom)*on/124'","zoompan=z='1.045':x='iw/2-(iw/zoom/2)':y='(ih-ih/zoom)*(1-on/124)'"],
    dinamico:["zoompan=z='min(zoom+0.0008,1.09)':x='(iw-iw/zoom)*on/124':y='(ih-ih/zoom)*(1-on/124)'","zoompan=z='min(zoom+0.0008,1.09)':x='(iw-iw/zoom)*(1-on/124)':y='(ih-ih/zoom)*on/124'","zoompan=z='1.075':x='(iw-iw/zoom)*on/124':y='ih/2-(ih/zoom/2)'","zoompan=z='1.075':x='(iw-iw/zoom)*(1-on/124)':y='ih/2-(ih/zoom/2)'"],
    cinematografico:["zoompan=z='min(zoom+0.00065,1.075)':x='iw/2-(iw/zoom/2)':y='(ih-ih/zoom)*on/124'","zoompan=z='if(eq(on,1),1.075,max(1.0,zoom-0.0006))':x='(iw-iw/zoom)*on/124':y='ih/2-(ih/zoom/2)'","zoompan=z='1.065':x='(iw-iw/zoom)*on/124':y='(ih-ih/zoom)*on/124'","zoompan=z='1.065':x='(iw-iw/zoom)*(1-on/124)':y='(ih-ih/zoom)*(1-on/124)'"],
    base:["zoompan=z='1.055':x='(iw-iw/zoom)*on/124':y='ih/2-(ih/zoom/2)'","zoompan=z='1.055':x='(iw-iw/zoom)*(1-on/124)':y='ih/2-(ih/zoom/2)'","zoompan=z='1.05':x='iw/2-(iw/zoom/2)':y='(ih-ih/zoom)*on/124'","zoompan=z='1.05':x='iw/2-(iw/zoom/2)':y='(ih-ih/zoom)*(1-on/124)'"]
  };
  const transitionGroups={elegante:['fade','dissolve','smoothleft','smoothright'],dinamico:['slideleft','slideright','slideup','slidedown','wipeleft','wiperight'],cinematografico:['fadeblack','fadewhite','dissolve','smoothup','smoothdown'],base:['fade','slideleft','slideright','smoothleft','smoothright']};
  const selected=Object.hasOwn(groups,style)&&style!=='base'?style:'automatico';
  const movements=(selected==='automatico'?Object.values(groups).flat():[...groups[selected],...groups.base]).sort(()=>Math.random()-.5);
  const transitions=[...new Set(selected==='automatico'?Object.values(transitionGroups).flat():transitionGroups[selected])].sort(()=>Math.random()-.5);
...
async function render(id,payload){
  ...
  try{
    ...
    const clips=[]; const direction=directionPlan(payload.style);
    for(let i=0;i<3;i++){
      const clip=path.join(dir,`clip-${i+1}.mp4`); clips.push(clip);
      const sceneFilter=payload.type==='images'
        ? `scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280,${direction.movements[i]}:d=125:s=720x1280:fps=25,setsar=1,format=yuv420p`
    ...
    joinArgs.push('-filter_complex',`[0:v][1:v]xfade=transition=${direction.transitions[0]}:duration=0.5:offset=4.5[x1];[x1][2:v]xfade=transition=${direction.transitions[1]}:duration=0.5:offset=9.0[vout]`,'-map','[vout]','-t','14','-r','25','-threads','1','-filter_threads','1','-c:v','libx264','-pix_fmt','yuv420p','-preset','veryfast','-crf','22','-an','-movflags','+faststart',music?silent:out);
    await run(joinArgs);
    ...
    if(music){
      ...
      await run(['-y','-i',silent,'-stream_loop','-1','-i',music,'-filter_complex','[1:a:0]volume=0.12,atrim=duration=14,asetpts=PTS-STARTPTS,afade=t=in:st=0:d=0.35,afade=t=out:st=13.3:d=0.7[aout]','-map','0:v:0','-map','[aout]','-map_metadata','-1','-c:v','copy','-c:a','aac','-b:a','160k','-shortest','-movflags','+faststart',out]);
...
app.post('/v1/jobs',auth,(req,res)=>{try{const {type,assets,music_url,style}=req.body;if(!['images','videos'].includes(type)||!Array.isArray(assets)||assets.length!==3)throw new Error('invalid_payload');assets.forEach(mediaUrl);if(music_url)mediaUrl(music_url);const editStyle=['automatico','elegante','dinamico','cinematografico'].includes(style)?style:'automatico';const id=crypto.randomUUID();jobs.set(id,{status:'pending',created_at:Date.now()});res.status(202).json({job_id:id,status:'pending'});setImmediate(()=>render(id,{type,assets,music_url,style:editStyle}));}catch(e){res.status(422).json({error:String(e.message||e)})}});
